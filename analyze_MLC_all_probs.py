...
def analyze():
    ...
    # result_dict = {}
    csvfile = open("./summary_mlccoco_acc_top1gt_all_probs.csv", 'w')
    # fieldnames = ['model_name', 'total', 'count_obj', 'ratio_obj', 'count_bg', 'ratio_bg',
    #               'count_intersection', 'ratio_obj_intersection', 'ratio_bg_intersection']
    ...
    for model_name in models[0:]:
        ...
        for i in range(0, len(list)):
            ...
            if True:
                ...
                if individual_count < 2:
                    result_obj[i] = True
                # arr = np.array([obj0_top1_l[i], obj127_top1_l[i], obj255_top1_l[i]])
                # if np.sum(arr == ground_truth_top1_l[i]) >= 2:
                #     result_obj[i] = True

        # with open(os.path.join('mlc_coco_analyze', "analyze_npz_result_obj_{}.txt".format(model_name)), 'w') as f:
        #     for i in result_obj:
        #         filename = list[i]
        #         f.write("{}\n".format(filename))
        #
        # with open(os.path.join('mlc_coco_analyze', "analyze_npz_result_bg_{}.txt".format(model_name)), 'w') as f:
        #     for i in result_bg:
        #         filename = list[i]
        #         f.write("{}\n".format(filename))
        #
        ...
        #
        # with open(os.path.join('top1gt_analyze', "analyze_npz_result_intersection_{}.txt".format(model_name)), 'w') as f:
        #     for i in common:
        ...
        # csvwriter.writerow({'model_name':model_name, 'total':total, 'count_obj':count_obj, 'ratio_obj':"{:.3%}".format(count_obj/total),
        #                     'count_bg':count_bg, 'ratio_bg': "{:.3%}".format(count_bg/total),
        #                     'count_intersection': count_intersection,
